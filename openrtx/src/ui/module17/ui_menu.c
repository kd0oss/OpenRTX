...
void _ui_drawSMSMenu(ui_state_t* ui_state)
{
    ...
    else if(ui_state->view_sms)
    {
    	...
    	if(rtx_getSMSMessage(state.currentSMSMessage, sender, message))
		{
    		...
    		curPos = 16 * state.currentSMSLine;
    		if(curPos >= mesgLen)
    			curPos = mesgLen;
    		charsLeft = mesgLen - curPos;
    		if(charsLeft > 0)
    		{
    			_ui_viewSubString(message, text, curPos, charsLeft < 18 ? charsLeft : 16);

    			gfx_print(layout.line1_pos, layout.message_font, TEXT_ALIGN_LEFT, color_white, text);

        		curPos = 18 * (state.currentSMSLine + 1);
        		if(curPos >= mesgLen)
        			curPos = mesgLen;
        		charsLeft = mesgLen - curPos;
        		if(charsLeft > 0)
        		...
        		curPos = 18 * (state.currentSMSLine + 2);
        		if(curPos >= mesgLen)
        			curPos = mesgLen;
        		charsLeft = mesgLen - curPos;
        		if(charsLeft > 0)
        		...
        		curPos = 18 * (state.currentSMSLine + 3);
        		if(curPos >= mesgLen)
        			curPos = mesgLen;
        		charsLeft = mesgLen - curPos;
        		if(charsLeft > 0)
    	...
    	else
        {
            ...
            _ui_drawMenuList(ui_state->menu_selected, _ui_getSMSEntryName);
        }
    }
    else
    {
