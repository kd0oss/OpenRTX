...
int processSerialmod17(void)
{
	...
	if (len != 1) {
		fprintf(stderr, "AMBE_Host: error when reading from Module17, errno=%d\n", errno);
		return 0;
	}

	if (buffer[0U] != AMBE3000_START_BYTE) {
		fprintf(stderr, "AMBE_Host: unknown byte from Module17, 0x%02X\n", buffer[0U]);
		return 1;
	}
...
int processSerialdongle(void)
{
	...
	if (len != 1) {
		fprintf(stderr, "AMBE_Host: error when reading from the dongle, errno=%d\n", errno);
		return 0;
	}
...
#else
				fprintf(stderr, "Usage: AMBE_Host [-d] [-i Module17 tty] [-v] [-x]\n");
#endif
				return 1;
		}
	}
